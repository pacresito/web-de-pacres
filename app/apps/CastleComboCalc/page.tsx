...
export default function CastleComboCalc() {
  ...
  }, [stepPlayer, stepPos, done, unlocked]);
  ...
  const saveResult = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/registro/castle-combo", {
        ...
        body: JSON.stringify({ password: passwordInput, date, players, scores: numericScores, totals, winner }),
      });
  ...
  return (
    <main className="min-h-screen flex flex-col items-center py-6 px-4" style={{ background: "#EBF5FB" }}>
      <div className="w-full" style={{ maxWidth: maxWidths[numPlayers] ?? "44rem" }}>
        ...
        {/* Password gate */}
        {!unlocked && (
          <div className="mb-5 bg-white rounded-2xl px-4 pt-4 pb-5 shadow border border-blue-100">
            <p className="text-center text-blue-800 text-sm font-bold mb-3">Introduce la clave para jugar</p>
            <div className="flex gap-2">
              <input
                ...
                placeholder="Clave"
                className="flex-1 border-2 rounded-xl px-3 py-3 text-base text-center focus:outline-none"
                style={{ borderColor: passwordError ? "#ef4444" : "#bfdbfe", minWidth: 0 }}
              />
              <button
                onClick={unlock}
                className="bg-blue-500 text-white rounded-xl px-5 text-xl font-bold"
              >
                →
              </button>
            </div>
              ...
              {POSITIONS.map((posItem, rowIdx) => {
                ...
                return (
                  <div
                    ...
                    style={{ gridTemplateColumns: cols, background: even ? "#b5d9ee" : "#ffffff" }}
                  >
