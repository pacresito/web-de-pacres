 ...
 */
export default function Minimapa({ id, marco, lon, lat, alto = 128 }: {
  ...
  return (
    <svg viewBox={`0 0 ${w.toFixed(2)} ${h}`} style={{ width: "100%", height: alto }} aria-hidden>
      <rect width={w} height={h} fill="var(--t-paper2)" />
      ...
      <rect width={w} height={h} fill="none" stroke="var(--t-rule2)" vectorEffect="non-scaling-stroke" />
    </svg>
  );
}
