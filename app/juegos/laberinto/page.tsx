...
import {
  BOARD_W, BOARD_H, CELL, BALL_R, START_X, START_Y,
  MAX_TILT, TILT_SPEED, GAME_DURATION,
...
export default function Laberinto() {
  ...
  function startLoop() {
    ...
    loopRef.current = function loop(time: number) {
      ...
      // Physics
      ({ bx: state.bx, by: state.by, vx: state.vx, vy: state.vy } =
        pasoBola(state, state.tiltX, state.tiltY, dt, state.segs));
