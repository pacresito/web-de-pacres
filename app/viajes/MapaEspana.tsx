...
// Geometría en data/viajes/espana-mapa.ts (generada, no a mano). Las tarjetas de
// provincia (fr-s1-provcard) son las que dan nombre y tap; el mapa es el escaparate.

export default function MapaEspana({ disponibles, onEntrar }: {
  disponibles: string[];               // provincias clicables (mismo peso), por name
  onEntrar: (region: string) => void;
}) {
  const { viewBox, inset, provincias } = MAPA_ESPANA;
  const activa = new Set(disponibles);
  const abrir = (name: string) => provincias.find((p) => p.name === name) && onEntrar(name);
        ...
        <div className="fr-s1-panel">
          ...
          <div className="fr-s1-provs">
            {disponibles.map((name) => (
              <button key={name} className="fr-s1-provcard" onClick={() => abrir(name)}>
