...
import type { DatosViajes, Destino, Rango, Restaurante } from "@/lib/viajes/tipos";
import { filtrarDestinos, type Filtros } from "@/lib/viajes/filtrar";
import { resumenFiltros } from "@/lib/viajes/resumen";
...
import type { Comida, Dia, Parada, Propuesta, Ritmo } from "@/lib/viajes/planificador/tipos";
import { serializarEncargo, type Encargo } from "@/lib/viajes/planificador/encargo";
...
export default function CrearViaje({ datos, filtros, inicial, onVolver }: {
  datos: DatosViajes;
  ...
  onVolver: () => void;     // volver al explorador (editar filtros)
}) {
  ...
  return (
    <div className="fr-s5-form">
      <div className="fr-s5-form-head">
        <button className="fr-s5-atras" onClick={onVolver} aria-label="Volver a los resultados">‹</button>
      ...
      <div className="fr-s5-form-card fr-tarjeta">
        <div className="fr-s5-heredados">
          ...
          <span>
            ...
            <button className="fr-s5-link" onClick={onVolver}>Editar</button>
          </span>
        </div>
...
// ------------------------------------------------------------------ PASO 2 · Propuestas
function Propuestas({ propuestas, encargo, rangoFechas, resumenHeredado, destinoPorSlug, restPorNombre, zona, onEditar }: {
  ...
  const compartir = async () => {
    const url = `${window.location.origin}/viajes?plan=${serializarEncargo(encargo)}`;
  ...
  const guardar = () => {
    try {
      const clave = "fr-viajes:guardados";
...
function DiaCard({ dia: d, fechaBase, zonaTxt, imprescindibles, destinoPorSlug, restPorNombre, abierto, onAbrir }: {
  dia: Dia;
  fechaBase: string;
  zonaTxt: string;
...
function ParadaItem({ parada: p, num, estrella, distanciaKm }: {
  parada: Parada;
  num: number;
  estrella: boolean;
  distanciaKm?: Rango;
  ...
  return (
    <Link href={`/viajes/${p.slug}`} className="fr-s5-item fr-s5-item--parada">
