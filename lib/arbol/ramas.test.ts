...
assert.strictEqual([...pertenencias.values()].filter((p) => p.porMatrimonio).length, 124, "las que entraron casándose");
...
assert.deepStrictEqual(
  ...
  "el reparto de las 416 por rama",
);
const total = [...pertenencias.values()].reduce((n, p) => n + p.ramas.length, 0);
assert.strictEqual(total, 432, "432 pertenencias para 416 personas: el mapa de áreas suma un 4 % de más");
assert.strictEqual([...pertenencias.values()].filter((p) => p.ramas.length > 1).length, 13, "los que están en más de una");
