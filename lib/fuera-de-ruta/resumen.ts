// Texto de los filtros activos: cada uno como chip quitable (filtrosActivos) y todos
// juntos como frase ("cascadas en Baztán, para niños").
import type { Desnivel, Filtros } from "./filtrar";
import { enumerar } from "./formato";

export type FiltroActivo = {
  etiqueta: string;
  sin: Filtros;     // los mismos filtros sin este
};

// Los ids del JSON van sin acento.
...
// Basta para los tipos reales (cascada→cascadas, mirador→miradores).
...
export function resumenFiltros(f: Filtros, nombreZona: (id: string) => string): string {
  const sujeto = f.tipo?.length ? enumerar(f.tipo.map(plural), "y") : "";
  const enZonas = f.zona?.length ? `en ${enumerar(f.zona.map(nombreZona), "y")}` : "";
  const cabeza = [sujeto, enZonas].filter(Boolean).join(" ");

  const frases: string[] = [];
  if (f.dificultad?.length) frases.push(`dificultad ${enumerar(f.dificultad, "o")}`);
  if (f.agua?.length) frases.push(`con ${enumerar(f.agua.map((a) => AGUA_TEXTO[a] ?? a), "o")}`);
  ...
  if (f.epoca?.length) frases.push(`en ${enumerar(f.epoca.map((e) => EPOCA_TEXTO[e] ?? e), "o")}`);
