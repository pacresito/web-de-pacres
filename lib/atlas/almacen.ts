...
import { aviso, calificar, HUECO, montar, siguiente, type Aviso, type Dato, type Mazo, type Nota, type Tarjeta } from "./srs";
...
export type Vista = {
  ...
  estrenando: boolean;
  /** Lo que dice la cabecera. Sale de aquí y no del render porque se mide contra el reloj, que
   *  en un render es impuro. */
  aviso: Aviso;
  /**
...
function avanzar(m: Mazo): Vista {
  ...
  const tarjeta = pais ? montar(m, pais, ahora) : null;
  return { tarjeta, estrenando: Object.keys(m).length === 0, aviso: aviso(m, tarjeta, ahora), n: ++servidas };
}
...
export async function sincronizar() {
  ...
  const ahora = Date.now();
  if (vista?.tarjeta) {
    // El aviso se rehace con el mazo que manda: lo hecho hoy incluye lo del otro dispositivo, y
    // quedándose con el del mazo de disco saldría contando media sesión.
    const tarjeta = montar(remoto, vista.tarjeta.pais, ahora);
    vista = { ...vista, tarjeta, aviso: aviso(remoto, tarjeta, ahora) };
  } else {
